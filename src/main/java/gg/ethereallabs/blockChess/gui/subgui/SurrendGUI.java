...
public class SurrendGUI extends BaseMenu {
    private final GameGUI gameGUI;
    private final Player player;
    private final Consumer<Boolean> choice;

    public SurrendGUI(GameGUI gameGUI, Player player, Consumer<Boolean> choice) {
        super("Surrend", 27);
        this.gameGUI = gameGUI;
        this.player = player;
        this.choice = choice;
        GameManager.playersSurrending.put(player.getUniqueId(), this);
    }

    @Override
    public void draw(Player p) {
        var acceptItem = createItem(
            BlockChess.mm.deserialize("<green>Surrender"), Material.GREEN_WOOL,
            List.of(BlockChess.mm.deserialize("<gray>Give your resignation").decoration(TextDecoration.ITALIC, false)),
            1
        );
        var cancelItem = createItem(
            BlockChess.mm.deserialize("<red>Cancel"), Material.RED_WOOL,
            List.of(BlockChess.mm.deserialize("<gray>Don't resign").decoration(TextDecoration.ITALIC, false)),
            1
        );
        if (inv != null) {
